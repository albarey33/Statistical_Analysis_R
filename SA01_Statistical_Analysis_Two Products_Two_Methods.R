...
########################################################################.
# STATISTICAL ANALYSIS  - SMALL SAMPLES T-TEST #####.
# Example: Two Products Sales, affected by two numerical variables
...
# As the skewness and kurtosisi all the numerical variables 
...
ggplot(data = melted_data, aes(x = Date, y = value, color = variable, group = variable)) +
  ...
  scale_color_manual(values = c("Product1" = "lightblue", "Product2" = "pink", "StockPrice" = "green", "MarketingSpend" = "purple")) +
  theme_minimal()
...
ggplot(data = melted_data[melted_data$variable %in% 
  ...
  #  geom_vline(xintercept = 0.83166, size = 0.01, color = "black") +
  labs(
...
ggplot(data = melted_data[melted_data$variable %in% 
       ...
       aes(x = value, fill = variable)) +
  scale_fill_manual(values = c("Product1" = "lightblue", "Product2" = "pink")) +
  #geom_histogram(alpha = 0.7, binwidth = 10, position = "identity") +
  geom_density(alpha = 0.2, aes(y = after_stat(scaled))) +
  #  geom_vline(xintercept = 0.83166, size = 0.01, color = "black") +
  labs(
    title = "Combined Histogram and Density Plot",
...
df_twoprods %>% group_by(paste(wday(df_twoprods$Date), df_twoprods$Day, sep = "-"), Method) %>% summarise(Total_Sales = sum(Sales))
...
ggplot(data = prod1_hist, 
  ...
  geom_boxplot() +
  labs(
    title = "Product1 vs Historical Sales",
...
#df_twoprods %>% filter(Method == "M01") %>% select(Product1)
#class(M01_Product1)
#class(df_twoprods[df_twoprods$Method == "M01","Product1"])
# * * 4.2.2 Two-Way ANOVA test 

melt(df_twoprods[,c("Date", "Method")], 
     id.vars = "Date") 

melted_data <- melt(df_twoprods[,c("Date", "Method", "Product1", "Product2")], 
                    id.vars = "Date") 
